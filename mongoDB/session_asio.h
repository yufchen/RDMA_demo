...
//	std::thread *SendThread = NULL;
	...
	uint64_t send_timeuse = 0;
	uint64_t recv_timeuse = 0;
    ...
    ASIOSession(TransportLayerASIO* tl, GenericSocket socket, bool isIngressSession) try
		...
		log() << "start a NEW Session" << std::endl;
	...
	void rdma_firstrecv(){
		...
		asio::read(_socket,asio::buffer(&tmp_con_data,sizeof(tmp_con_data)),ec);
		rdma_res.remote_props.addr=tmp_con_data.addr;
        rdma_res.remote_props.rkey=tmp_con_data.rkey;
        rdma_res.remote_props.qp_num=tmp_con_data.qp_num;
        rdma_res.remote_props.lid=tmp_con_data.lid;

		log() << "Remote address   = " << rdma_res.remote_props.addr << std::endl;
		log() << "Remote rkey	   = " << rdma_res.remote_props.rkey << std::endl;
		log() << "Remote QP number = " << rdma_res.remote_props.qp_num << std::endl;
		log() << "Remote LID	   = " << rdma_res.remote_props.lid << std::endl;
	...
	void rdma_firstsend(){
		std::error_code ec;
		struct cm_con_data_t tmp_con_data;
		tmp_con_data.addr = rdma_res.local_props.addr;          
		tmp_con_data.rkey = rdma_res.local_props.rkey;
		tmp_con_data.qp_num = rdma_res.local_props.qp_num;
    	tmp_con_data.lid = rdma_res.local_props.lid;
		asio::write(_socket,asio::buffer(&tmp_con_data, sizeof(tmp_con_data)),ec);
		log() << "Local address   = " << rdma_res.local_props.addr << std::endl;
		log() << "Local rkey	  = " << rdma_res.local_props.rkey << std::endl;
		log() << "Local QP number = " << rdma_res.local_props.qp_num << std::endl;
		log() << "Local LID 	  = " << rdma_res.local_props.lid << std::endl;	
		return;
	}
	
	Future<Message> rdma_sourceMessageImpl(){
		size_t msgsize = rdma_res.Recv();
		auto buffer = SharedBuffer::allocate(msgsize);
		memcpy(buffer.get(), (char*)rdma_res.recvbuf, msgsize);
    ...
    StatusWith<Message> sourceMessage() override {
		...
		ensureSync();
		if(firstSource){
			log() << "Source0" << std::endl;
			rdma_firstrecv();				
			rdma_res.connect_qp();
			firstSource = false;
			rdma_res.is_ready = true;
        	return sourceMessageImpl().getNoThrow();			
		}
		else if(rdma_res.is_ready){
			log() << "SourceRDMA" << std::endl;
			return rdma_sourceMessageImpl().getNoThrow();
		}
		else{
			log() << "Source" << std::endl;
        	return sourceMessageImpl().getNoThrow();	
    ...
    Future<Message> asyncSourceMessage(const transport::BatonHandle& baton = nullptr) override {
		...
		log() << "ASource" << std::endl;
		if(baton){
			log() << "addSession In" << std::endl;
			baton->addSession(*this, Baton::Type::In);
		}			
    ...
    Status sinkMessage(Message message) override {
		...
		if(firstSink){
			...
			log() << "SinkRDMA" << std::endl;
			std::error_code ec;
			rdma_res.Send((void*)(message.buf()), (size_t)(message.size()));
			return futurize(ec).getNoThrow();
		}
    ...
    Future<void> asyncSinkMessage(Message message,
		...
		if(baton){
			log() << "addSession Out" << std::endl;
			baton->addSession(*this, Baton::Type::Out);
        }			
...
private:
    ...
    Future<void> opportunisticRead(Stream& stream,
        ...
        if (MONGO_FAIL_POINT(transportLayerASIOshortOpportunisticReadWrite) &&
            ...
            size = asio::read(stream, buffers, ec);//一般执行这个
        }
